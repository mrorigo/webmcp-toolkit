...
**Goal:** Prove that an in-page ReAct loop can autonomously drive a form using the zero-cost Chrome Prompt API (`window.ai`).
...
- **1.3 Chrome Prompt API Integration:** Use `window.ai.languageModel` (as sketched in `experiments/llm-tool-call.js`) to feed the DOM semantic text and a list of internal subagent tools (`input_text`, `click_element`, `wait`, `done`).
...
- **4.2 Edge LLM Fallbacks:** Implement a clean adapter pattern so if `window.ai` is not available, the developer can provide a lightweight cloud or edge model abstraction.
